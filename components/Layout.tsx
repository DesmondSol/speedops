...
export const Layout: React.FC<Props> = ({ children, activeTab, onTabChange }) => {
  ...
  return (
    <div className="flex min-h-screen bg-[#050505] text-white">
      {/* Desktop Sidebar */}
      <aside 
        className={`fixed left-0 top-0 h-full bg-[#0F0F0F] border-r border-white/10 z-50 transition-all duration-300 hidden md:block ${expanded ? 'w-64' : 'w-20'}`}
        onMouseEnter={() => setExpanded(true)}
        onMouseLeave={() => setExpanded(false)}
      >
        <div className="flex items-center p-6 mb-8 overflow-hidden">
          <div className="min-w-[32px] h-8 bg-[#FF6A00] flex items-center justify-center font-bold text-black rounded-sm mr-4">S</div>
        ...
        <nav className="px-3 space-y-2">
          {sidebarItems.map((item) => (
            ...
            >
              ...
              {expanded && <span className="ml-4 font-medium">{item.label}</span>}
            </button>
          ))}
        </nav>
      </aside>
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          <div className="flex items-center gap-6">
            <div>
              ...
              <p className="text-gray-500 font-mono text-[10px] md:text-sm uppercase tracking-widest">Operational Intelligence / Alpha_v1.2</p>
            </div>
            <div className="hidden lg:flex items-center gap-2 px-3 py-1 bg-[#FF6A00]/5 border border-[#FF6A00]/20 rounded-full">
               ...
               <span className="text-[9px] font-mono font-bold text-[#FF6A00] uppercase tracking-widest">Live Cloud Link</span>
            </div>
          </div>
          
          <div className="flex items-center gap-3">
            <div className="relative group flex-1 md:flex-none">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-[#FF6A00]" size={16} />
              <input 
                type="text" 
